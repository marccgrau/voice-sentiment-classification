...
class AdaptedAttentionBlock(nn.Module):
    def __init__(self, encoder_block, input_dim, bottleneck_dim):
        super(AdaptedAttentionBlock, self).__init__()
        self.attn = encoder_block.attn
        self.attn_adapter = Adapter(input_dim, bottleneck_dim)
        self.attn_ln = encoder_block.attn_ln
        self.cross_attn = encoder_block.cross_attn
        self.cross_attn_adapter = (
            Adapter(input_dim, bottleneck_dim) if self.cross_attn is not None else None
        )
        self.cross_attn_ln = encoder_block.cross_attn_ln
        self.mlp = encoder_block.mlp
        self.mlp_adapter = Adapter(input_dim, bottleneck_dim)
        self.mlp_ln = encoder_block.mlp_ln
    ...
    def forward(
        ...
        x = x + self.attn_adapter(
            self.attn(self.attn_ln(x), mask=mask, kv_cache=kv_cache)[0]
        )
        if self.cross_attn:
            x = x + self.cross_attn_adapter(
                self.cross_attn(self.cross_attn_ln(x), xa, kv_cache=kv_cache)[0]
            )
...
class AdaptedEncoder(nn.Module):
    def __init__(
        self, encoder_blocks, prompt_len, input_dim, bottleneck_dim, adapt_list=None
    ):
        ...
        for i in range(self.block_num):
            if i in self.adapt_list:
                self.adpated_blocks[i] = AdaptedAttentionBlock(
                    encoder_blocks[i], input_dim, bottleneck_dim
                )
        ...
        self.prompt = nn.Parameter(
            torch.zeros(self.prompt_len, self.dim * self.block_num), requires_grad=True
        )
    ...
    def forward(self, x: Tensor):
        ...
        for i in range(self.block_num):
            prompt = (
                self.prompt[:, i * self.dim : (i + 1) * self.dim]  # noqa: E203
                .unsqueeze(0)
                .expand(bs, self.prompt_len, self.dim)
            )
...
class WhisPAr(nn.Module):
    def __init__(self, cls_num, audio_len, prompt_len, bottleneck_dim, adapt_list=None):
        ...
        self.encoder_blocks = AdaptedEncoder(
            whisper_encoder.blocks,
            self.prompt_len,
            self.dim,
            bottleneck_dim,
            adapt_list,
